  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "Extract key information from this resume in JSON format. The JSON output should include the keys personal_info, skills, work_experience, education,certifications and and competitiveProfiles. Ensure personal_info includes fields: name, email, phone, and location (If missing try to infer it from work experience first, if not found there infer it from education,if not found there use empty strings). Ensure work_experience is an array of objects where each object includes at least company, title,work role or job title and send it with postion as the key and duration (use empty strings for any missing details). For education, include institution, degree, field, and year if available. If a section has no data, return an empty array or an object with empty fields as appropriate.Ensure all keys use camelCase formatting: personalInfo, skills, workExperience, education, certifications, and competitiveProfiles.Ensure competitiveProfiles includes any and all references to platforms such as LeetCode, HackerRank, Kaggle, Codeforces, and CodeChef.If a profile includes Rating,Rank or score or a link, Include it as well. Do not include any markdown formatting in your output.If the field is not mentioned under education go over the workExperience to determine the field of work of the individual and put it under field under education"
        },
        {
          role: "user",
          content: text
